"""Import the audited ScienTI events snapshot into Kahi."""
...
EVENT_FIELDS = {
    ...
    "external_urls", "date_held", "year_held", "author_count", "authors",
    "ranking", "groups",
}
...
SCALAR_FIELDS = {"abstract", "date_held", "year_held"}
...
class Kahi_minciencias_opendata_events(KahiBase):
    """Consume only the events member of an audited six-entity release."""
    ...
    def __init__(self, config):
        ...
        source_config = config["minciencias_opendata_events"]
        forbidden = {
        ...
        if legacy:
            raise ValueError(
                "legacy event options are no longer supported: {}".format(
                    ", ".join(legacy)
                )
            )
        ...
        self.runs = self.db[source_config.get(
            "runs_collection", "minciencias_opendata_events_runs"
        )]
        self._configure_snapshot(source_config)
        self.run_id = "events:{}".format(self.release_name)
    ...
    def _configure_snapshot(self, source_config):
        ...
        collection_name = str(collections.get("events") or "")
        evidence = (audit.get("evidence") or {}).get("events") or {}
        ...
        valid = (
            ...
            and evidence.get("materialization_run") == runs.get("events")
        ...
        if not valid:
            raise RuntimeError("six-entity event snapshot is not proven and published")
        documents = self.source_db[collection_name].count_documents({})
        if documents != int(evidence.get("documents") or -1):
            raise RuntimeError("event snapshot count changed after release audit")
        self.release_name = release_name
        self.audit_name = audit["_id"]
        self.materialization_run = runs["events"]
    ...
    def _validate_document(document):
        ...
        if (
            EVENT_FIELDS - fields
            or fields - ({"_id"} | EVENT_FIELDS | SOURCE_ONLY_FIELDS)
        ):
            raise RuntimeError("event snapshot does not follow the Kahi schema")
    ...
    def _source_codes(document):
        ...
        if any(not COD_RH_RE.fullmatch(code) for code in author_codes):
            raise RuntimeError("event snapshot contains an invalid COD_RH")
        if any(not GROUP_RE.fullmatch(code) for code in group_codes):
            raise RuntimeError("event snapshot contains an invalid group code")
    ...
    def _reference_maps(self, batch):
        ...
        if missing_people:
            raise RuntimeError(
                "unresolved event authors: {}".format(", ".join(missing_people[:10]))
            )
        ...
        if missing_groups:
            raise RuntimeError(
                "unresolved event groups: {}".format(", ".join(missing_groups[:10]))
            )
    ...
    def _event(self, source, people, groups):
        output = {field: deepcopy(source[field]) for field in EVENT_FIELDS}
    ...
    def _process_batch(self, batch, counters):
        ...
        for source in batch:
            ...
            output.append(self._merge(current, self._event(source, people, groups)))
    ...
    def process_snapshot(self, previous):
        ...
        for document in cursor:
            ...
            if self.verbose > 0:
                print("INFO: imported {} audited events".format(counters["processed"]))
            batch.clear()
        if batch:
            self._process_batch(batch, counters)
            counters["processed"] += len(batch)
        ...
        if counters["processed"] != self.source_documents:
            raise RuntimeError("event snapshot changed while it was being imported")
        return counters
    ...
    def run(self):
        ...
        if previous and any(
            ...
            raise RuntimeError("event import run exists with different source evidence")
