"""Import the audited ScienTI patents snapshot into Kahi."""
...
PATENT_FIELDS = {
    "titles", "updated", "types", "external_ids", "external_urls",
...
class Kahi_minciencias_opendata_patents(KahiBase):
    """Consume only the patents member of an audited six-entity release."""
    ...
    def __init__(self, config):
        ...
        source_config = config["minciencias_opendata_patents"]
        forbidden = {
        ...
        if legacy:
            raise ValueError(
                "legacy patent options are no longer supported: {}".format(
                    ", ".join(legacy)
                )
            )
        ...
        self.runs = self.db[source_config.get(
            "runs_collection", "minciencias_opendata_patents_runs"
        )]
        self._configure_snapshot(source_config)
        self.run_id = "patents:{}".format(self.release_name)
    ...
    def _configure_snapshot(self, source_config):
        ...
        collection_name = str(collections.get("patents") or "")
        evidence = (audit.get("evidence") or {}).get("patents") or {}
        ...
        valid = (
            ...
            and evidence.get("materialization_run") == runs.get("patents")
        ...
        if not valid:
            raise RuntimeError("six-entity patent snapshot is not proven and published")
        documents = self.source_db[collection_name].count_documents({})
        if documents != int(evidence.get("documents") or -1):
            raise RuntimeError("patent snapshot count changed after release audit")
        self.release_name = release_name
        self.audit_name = audit["_id"]
        self.materialization_run = runs["patents"]
    ...
    def _validate_document(document):
        fields = set(document)
        if (
            PATENT_FIELDS - fields
            or fields - ({"_id"} | PATENT_FIELDS | SOURCE_ONLY_FIELDS)
        ):
            raise RuntimeError("patent snapshot does not follow the Kahi schema")
    ...
    def _source_codes(document):
        ...
        if any(not COD_RH_RE.fullmatch(code) for code in author_codes):
            raise RuntimeError("patent snapshot contains an invalid COD_RH")
        if any(not GROUP_RE.fullmatch(code) for code in group_codes):
            raise RuntimeError("patent snapshot contains an invalid group code")
    ...
    def _reference_maps(self, batch):
        ...
        if missing_people:
            raise RuntimeError(
                "unresolved patent authors: {}".format(", ".join(missing_people[:10]))
            )
        ...
        if missing_groups:
            raise RuntimeError(
                "unresolved patent groups: {}".format(", ".join(missing_groups[:10]))
            )
    ...
    def _patent(self, source, people, groups):
        output = {field: deepcopy(source[field]) for field in PATENT_FIELDS}
    ...
    def _process_batch(self, batch, counters):
        ...
        for source in batch:
            ...
            output.append(self._merge(current, self._patent(source, people, groups)))
    ...
    def process_snapshot(self, previous):
        ...
        for document in cursor:
            ...
            if self.verbose > 0:
                print("INFO: imported {} audited patents".format(counters["processed"]))
            batch.clear()
        if batch:
            self._process_batch(batch, counters)
            counters["processed"] += len(batch)
        ...
        if counters["processed"] != self.source_documents:
            raise RuntimeError("patent snapshot changed while it was being imported")
        return counters
    ...
    def run(self):
        ...
        if previous and any(
            ...
            raise RuntimeError("patent import run exists with different source evidence")
